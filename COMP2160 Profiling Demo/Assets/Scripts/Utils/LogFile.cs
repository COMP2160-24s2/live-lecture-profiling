...
namespace WordsOnPlay.Utils
{
	public class LogFile : MonoBehaviour
	{
		...
		private string path = Path.Combine(
			System.Environment.GetFolderPath(System.Environment.SpecialFolder.Personal), "Logs");

		// The base name for log files
		[SerializeField] private string name = "log";

		// the format for the filename. 
		// The {0} is filled with the path above
		// the {2:yyyy-MM-dd-HH-mm-ss} is filled with the current data/time in a sortable format.
		...
		void Awake()
		{
			...
			Debug.Log($"Created log file: {filename}");
